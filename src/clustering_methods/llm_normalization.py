...
def cluster_via_llm_normalization(
    ...
    normalized_texts = [normalized_map.get(i, documents[i]) for i in range(n_samples)]
    concat_features = np.hstack(
        [
            normalize(features, axis=1, norm="l2"),
            llm_service.get_embeddings(normalized_texts),
        ]
    )
